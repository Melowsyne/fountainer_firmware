...
(scope=`control`), datapoint reports and command dispatch — as a FreeRTOS task.

Self-contained: the connection to the application/`data_store`/`command` is made via
**callbacks**, not via hard dependencies.

## Modules

| File | Purpose | Host-testable |
|-------|---------|:------------:|
...
| `fp_session.{h,c}` | Handshake/negotiation, auth, dispatch, replies | ✓ |
| `fp_ws.{h,c}` | `esp_websocket_client` wrapper (wss, CA, bearer) | — (ESP-IDF) |
| `fp_task.{h,c}` | FreeRTOS task, timer, `fountain_proto_start()` | — (ESP-IDF) |
...
Place the component in the project (e.g. as `components/clientside_protocol`). The
component manager pulls in `esp_websocket_client` (see `idf_component.yml`).
...
static void on_command(const cJSON *m, cJSON *res, void *u) {
    /* map command onto the data_store/command module, then: */
...
- **Wi-Fi:** establish via the existing `wlan_com`; then `fountain_proto_start()`.
