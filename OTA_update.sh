#!/bin/bash
# Fountainer: deploy an OTA image to the server.
#
...
if [ "$BASENAME" != "fountain-$VERSION.bin" ]; then
    echo "NOTE: file name '$BASENAME' != embedded version -> target is fountain-$VERSION.bin"
fi

cp "$SRC" "$TARGET"
echo "==> Deployed: $TARGET (embedded version: $VERSION)"
echo "==> The server offers the update at the next device connect"
