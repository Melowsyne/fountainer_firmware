...
# clientside_protocol component). The DP mutex mock is a REAL pthread mutex
# so that the concurrency stress test checks real serialization.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
ESP="$(cd "$HERE/../.." && pwd)"
OUT="${TMPDIR:-/tmp}/test_datapoints"

# Find the cJSON source: prefer the system, else framework-espidf* (IDF 5.5.x).
CJDIR=""
CJSON_INC=()
CJSON_SRC=()
