...
/* ----- LE serialisation (unaligned-safe, toolchain-neutral) ------------- */
 ...
 * because head then naturally wraps to 0). */
static bool at_wrap_gap(const log_ring_t *r, uint16_t pos)
{
    return r->buf[pos] == 0u;
}

/* Discards the oldest record (a wrap gap before it is released as well). */
static void discard_oldest(log_ring_t *r)
{
    ...
    if (r->count == 0) {           /* empty: normalise positions */
        r->head = 0;
        r->tail = 0;
        r->used = 0;
    }
                       ...
                       const int32_t *args, uint8_t argc, const char *text)
{
     ...
     * Each iteration discards at least one record -> hard-bounded
     * (needed <= 78, min record 15: few rounds, critical-section-safe). */
    for (;;) {
        ...
        if (r->count == 0 || r->head >= r->tail) {
            ...
            if (end_space >= needed) break;                  /* fits at the end*/
        ...
        /* head < tail: free region [head, tail) is contiguous and
...
/* Validation (Entwurf §33): corrupt size fields abort the iteration
