...
### HW2.x — fountainer_hw_24v PCB (Rev. 1, runtime-selected via factory `hw_rev`)

| GPIO | Function | Notes |
|---|---|---|
| 7 | SSR (pump) | low-side switch Q1, LEDC PWM |
| 17 / 18 | I2C SDA / SCL → ADS1115 | input divider 10k/15k (factor 0.6, `flSensorVdiv`) |
| 15 / 16 | status LEDs red / green | active low |
| 6 | DHT dummy | not routed — no climate sensor on this board |
| 5 | CAN_TX → TJA1051T/3 TXD | TWAI controller |
| 4 | CAN_RX ← TJA1051T/3 RXD | push-pull output of the transceiver — never driven |
| 21 | CAN_PWDN → TJA1051T/3 S | HIGH = silent (boot default), LOW = normal (`hal_can_standby_set`) |
| 43 / 44 | UART0 console via CP2102N | the only console on this board |

CAN bus on J3 (CANH/CANL), common-mode choke FL3, ESD clamp NUP2105,
120 Ω termination R24 via DIP switch S3. Protocol: [CANopen.md](CANopen.md).
