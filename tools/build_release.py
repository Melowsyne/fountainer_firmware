...
to EVERY device; a test image there would go via OTA onto the production device).
"""
import argparse
import hashlib
import json
...
def embedded_version(bin_path: Path) -> str:
    """esp_app_desc_t.version: 32 bytes from offset 48 (0x30)."""
...
def main():
    ...
    ap.add_argument("--allow-dirty", action="store_true",
                    help="Build despite uncommitted changes (noted in the manifest)")
    args = ap.parse_args()
    ...
    if embedded_version(fw_bin) != version:
        ...
        run([PIO, "run", "-e", args.env], cwd=FW_ROOT, env=env)
        if embedded_version(fw_bin) != version:
            sys.exit("ERROR: image version still wrong after clean rebuild")

    # Signature is mandatory (sign_firmware.py aborts on esp32s3_prod anyway,
    ...
    # Collect artifacts — flash_device.py never depends on the build directory.
    ...
    # Take the flash parameters along for the production station.
