 ...
 * (Entwurf/drucksensor_datenstruktur.md). Fully generic:
 * no ESP-IDF, no FreeRTOS, no heap; synchronisation is the
 * caller's job.
 *
 * Deviation from the design (§11/§25), agreed with the user:
 ...
 * numbers remain assigned without gaps, `overwritten` counts every
 * eviction and first_seq_available makes the gap explicit.
 * ============================================================= */

typedef struct {
    uint32_t sequence;       /* monotonic per boot, start 1               */
...
typedef struct {
    ...
    uint16_t count;
    uint32_t next_seq;       /* seq of the NEXT sample (start 1)          */
...
/* Non-destructive read of all samples with seq > since_seq (oldest
 * first, at most `max` items). Seqs are gapless -> O(1) positioning. */
