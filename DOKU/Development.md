...
- For signed builds: the CA directory checked out **next to** the repo
  (`../CA`), containing `ota_signing/ota_signing_key.pem`. Without it the
...
`platformio.ini` defines two envs:
...
- `test_co_core.c` — the CANopen slave core (`src/components/canopen/`):
  boot-up/heartbeat encoding, NMT transitions, SDO expedited + segmented
  transfers (toggle, aborts, timeout), TPDO/RPDO mapping via SDO, SYNC,
  EMCY — byte-exact against CiA 301 (139 checks)
...
- All source files carry the proprietary copyright header; comments and
