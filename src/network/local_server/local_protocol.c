 ...
 * (firmware_server.md §4/§5, implementation plan AP5).
 *
 * The ESP32 remains the Fountain DEVICE: for each accepted connection IT
 * sends hello; the maintenance client (Fountain SERVER role) replies
 * hello_ack, verifies the signed ota_check proof and switches to RUNNING
 * with ota_none (§48 variant A — zero protocol change).
 *
 ...
 * identical feature set as the web UI. The isolation contract is
 * preserved: the task_com_apply_* variants do NOT call power_mgmt_activity_note()
...
static const fp_config_t *local_cfg_get(void)
{
    if (!s_stLocalCfg.device_id) {
         ...
         * (header comment — cloud isolation). */
    }
    return &s_stLocalCfg;
}
