...
/* ----- TX: the only send path (httpd task via queue_work) ---------------- */
typedef struct {
    int    sockfd;
    size_t len;
    char   data[];
...
static void tx_work(void *arg)
{
    ...
    /* We are already running in the httpd task: only the direct sender is
     * allowed. httpd_ws_send_data() would queue another work item here and
...
{
    /* CAUTION IDF pin: in IDF 5.5.3 httpd calls the handler after the WS
     * handshake with method==HTTP_GET (httpd_uri.c). IDF 6 no longer does
     * that (return before uri->handler; instead
     * CONFIG_HTTPD_WS_POST_HANDSHAKE_CB_SUPPORT) — this branch would never
     * run there: no slot, no sess_ctx. Adapt on IDF upgrade! */
    ...
    /* Size limit BEFORE any processing (§26); >= because of NUL for cJSON. */
    ...
    /* Drain buffer for rejected frames: ONE is enough, the handler runs
     * exclusively in the httpd task (RAM is the bottleneck, M1). */
    static uint8_t s_aucDrain[LOCAL_MAX_FRAME_SIZE];

    if (pkt.type != HTTPD_WS_TYPE_TEXT) {                 /* text only (§50) */
        /* The payload MUST be drained from the stream — otherwise the frame
         * bytes stay unread, the next WS header is parsed incorrectly and
         * the connection dies. (Found by fuzzing: a binary frame killed the
         * session.) Control frames (ping/pong/close) are handled by httpd
         * itself; what lands here is mainly data frames of type BINARY. */
        if (pkt.len > 0) {
    ...
    /* Rate limit: dropping is cheap, repeated abuse closes (§33). */
...
/* ----- Start/stop on the Wi-Fi lifecycle --------------------------------- */
...
static void server_start(void)
{
     ...
     * httpd however needs AT LEAST 2 sockets to work reliably (with
     * max_open_sockets=1 esp_https_server does not even accept the FIRST
     * connection cleanly). The one extra socket also allows a client beyond
     * the limit to be rejected CLEANLY via the semaphore (ESP_FAIL in the
     * GET handler) instead of leaving it hanging at TCP level. */
