...
/* Timed-on is DECOUPLED from the Fon_Max_On_Time fault limit
...
{
    pm_config_t c = cfg_default();
    c.dry_detect_ms = 600000;            /* dry guard out of the way (10 min) */
    c.min_on_ms = 1000; c.min_off_ms = 400;
    c.max_on_ms = 60000;                 /* 60 s — deliberately SMALLER than duration */
    pm_t pm; uint64_t now = 1000;
    pm_init(&pm, &c);
    run(&pm, &now, 1.0f, 2, 200);

    /* (a)+(b) A 90-s timed-on survives the 60-s max_on limit and counts
     * down to the REQUESTED duration. */
    ...
    /* (d) Unlimited manual on (duration 0) STILL latches at max_on_ms —
     * the safety limit is not softened. */
    run(&pm, &now, 1.0f, 30, 200);       /* pause for min_off               */
    CHECK(pm_request_on(&pm, 0, now), "unlimited manual on accepted");
    run(&pm, &now, 1.0f, 320, 200);      /* > 60 s run time                 */
    CHECK(pm.state == PM_STATE_FAULT && pm.fault == PM_FAULT_MAX_RUNTIME,
          "unlimited on still latches MAX_RUNTIME at max_on_ms");
}


/* Regression (stress test 2026-09-23): a remote request stamps
 * pump_start_ms with a NEWER timestamp than the ullNow of the running
 * cycle (time is fetched at cycle start, the lock only later).
 * Previously: unsigned underflow of the run time -> MAX_RUNTIME immediately. */
static void test_request_newer_than_cycle_time(void)
{
    pm_t pm; pm_config_t c = cfg_default(); pm_init(&pm, &c);
    uint64_t now = 0;
    run(&pm, &now, 1.0f, 3, 100);                       /* INIT -> IDLE     */
    uint64_t cycle_now = now;                           /* cycle time       */
    CHECK(pm_request_on(&pm, 0, cycle_now + 40), "on accepted with newer timestamp");
    pm_update(&pm, 1.0f, true, cycle_now);              /* older cycle      */
    CHECK(pm.state == PM_STATE_RUNNING && pm_relay_get(&pm) && pm.fault == PM_FAULT_NONE,
          "older cycle time does not trip MAX_RUNTIME");
    CHECK(pm.m.run_time_s == 0 && pm.m.remaining_s == c.max_on_ms / 1000,
          "run time clamps to 0 instead of wrapping");
    run(&pm, &now, 1.0f, 20, 100);                      /* 2 s normal, on   */
    CHECK(pm.state == PM_STATE_RUNNING && pm.m.run_time_s >= 1, "keeps running normally");
}
...
int main(void)
{
    ...
    test_request_newer_than_cycle_time();
