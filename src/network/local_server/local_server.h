 ...
 * Fountain DEVICE: for each accepted connection IT starts the hello
 ...
 * Wi-Fi (EVT_WLAN_CONNECTED/DISCONNECTED). Read-only phase: the session
 * automatically answers dp_write/command/log_read with "rejected"
 ...
 * creates slots/queues/pool and subscribes to the Wi-Fi events. */
...
/* TX path for the protocol layer (AP5): serialized via the httpd task
 * (one TX owner per socket, §41). conn_id invalidates sends to a slot
