...
#
# WHY this script: the tests need cJSON + mbedTLS. A naive build against
# the system runtime lib (libmbedcrypto.so.7) with the Espressif mbedTLS
# HEADERS mixes two ABIs — it compiles and LINKS, but produces a silent
# WRONG hash (SHA256 context layout differs) and the golden test then
# falsely reports MISMATCH. This script builds cJSON AND mbedTLS from
# ONE source (the PlatformIO ESP-IDF toolchain) so that headers and code
# match each other. Alternatively (USE_SYSTEM=1) against the system dev packages.
#
...
if [ "${USE_SYSTEM:-0}" = "1" ]; then
    ...
    # Find cJSON in an installed IDF. Note: since IDF 6 there is no
