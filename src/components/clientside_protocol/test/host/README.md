...
depend only on **cJSON** + **mbedTLS** and can therefore be tested on an ordinary Linux host
— without ESP-IDF. `fp_ws`/`fp_task` are ESP-IDF-specific and are not built
here.
...
> **Why not simply link against the system lib?** A build with the
...
> layout differs, the SHA256 silently produces a **wrong** hash, and the
> golden test falsely reports `MISMATCH`. The runner avoids this by taking
> headers and code from the same tree.

With system dev packages installed (`libcjson-dev libmbedtls-dev
...
## Manual build (only with system dev packages, ABI-consistent)
...
Both tests end with exit 0 and print `GOLDEN VECTOR : OK` and `SESSION TEST OK` respectively.

Via Docker (without host installation):
