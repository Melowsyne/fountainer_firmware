...
"""create_devices.py — creates new device records for series production.
    ...
    python3 tools/create_devices.py --batch 2026-08-A \
        ...
        --hw-revision HW1.0 --server SERVER_IP [--port 8443]

Per device: next free FNT number (never reused), device_id in the
existing-device scheme esp32-<mac> (Wi-Fi STA MAC of the specific device,
mandatory --mac, one per device), secrets directory under
<secrets_root>/FNT-xxxxxx/ (HMAC key + combo certificate clientAuth+serverAuth
via ../DO_NOT_COMMIT/CA/issue_device_cert.sh v3_device — the device needs
the server role for the local WSS maintenance access), bearer token, entry
in production/devices.json (status "allocated").

Safety rules (Entwurf/serienfertigung.md §13/§27): uniqueness of
...
# human-readable in the hex dump and collision-free with the MAC-based legacy
...
def check_unique(db):
    """Aborts on every violation — never continue silently."""
...
def main():
    ...
    ap.add_argument("--mac", action="append", required=True, metavar="MAC",
                    help="Wi-Fi STA MAC of the device (one per device) — becomes "
    ...
    for i, mac in enumerate(macs):
        ...
        # Combo cert clientAuth+serverAuth (v3_device): the device is cloud
