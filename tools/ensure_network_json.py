...
# device-specific identity (client certificate/key) comes from the factory
# NVS partition, NEVER from the binary. The shared CA stays in.
if env["PIOENV"] == "esp32s3_prod":  # noqa: F821
    if not ca:
        raise SystemExit("ensure_network_json: ERROR — ca.crt.pem missing, "
                         "production build without CA not allowed")
    crt = key = ""
    print("ensure_network_json: esp32s3_prod — client certificate/key are "
          "NOT embedded (identity comes from the factory partition)")
