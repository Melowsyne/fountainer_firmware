 ...
 * (Entwurf/logger_datenstruktuer.md). Fully generic:
 * no ESP-IDF, no FreeRTOS, no heap — synchronisation is the
 * caller's job (short critical section around push/read).
 *
 * Record layout (explicitly serialised byte by byte, LE):
 *   off len field
 ...
 *   4   2   event_id
 *   6   4   seq          (u32 — wire format needs u32, not u16)
 *   10  4   uptime_ms
 ...
 * ones give way (overwritten counts them, first_seq moves up — the
 * loss is thus never silent). Records are never split across the
 * buffer end (wrap marker record_size == 0).
 * ============================================================= */
...
typedef struct {
    ...
    uint16_t  used;          /* used bytes incl. wrap gap                  */
    uint16_t  count;         /* valid records                              */
    uint32_t  next_seq;      /* seq of the NEXT record (start 1)           */
    uint32_t  first_seq;     /* seq of the oldest record; == next_seq when empty */
    uint32_t  overwritten;   /* evicted records since init                 */
} log_ring_t;

/* Decoded view of a record; text points into the ring buffer and is
 * only valid as long as the caller locks the ring against writers. */
typedef struct {
    uint8_t     level;
    uint8_t     module;
    uint8_t     flags;
...
/* Writes a record (text > LOG_RING_MAX_TEXT is truncated and marked
