 ...
 * without holding the device in HIGH-power mode (cloud wrapper below). */
void task_com_apply_command(const cJSON *cmd_msg, cJSON *result_out)
{
    ...
    /* Distinguish the error cause (previously everything was "unknown_command"
     * — misleading when a KNOWN command only fails on the device logic,
     * e.g. manual switch-on with a latched pump fault). */
...
/* Public so that the local maintenance session uses the same log-pull
 * logic (no cloud-specific state). Signature = fp_config callback. */
 ...
 * (ring is RAM-only). Cloud + local maintenance session share this function. */
                                 ...
                                 uint32_t max_samples, void *user)
{
    ...
    log_stats_t stLog;
    logging_stats_get(&stLog);                  /* reuse boot_id           */
...
bool task_com_start(void)
{
     ...
     * fallback (factory partition or, without one, the legacy SERVER_IP).
