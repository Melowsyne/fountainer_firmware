 ...
 * Entwurf/logger_datenstruktuer.md). Pure C — no mock needed. Checks the
 * roundtrip of all fields, truncation, wrap marker at all offsets (text-length
 * sweep), eviction invariants, iter_seek, clear-keeps-seq and a randomized
 * 100k stress against a shadow model.
 */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
static void check_invariants(const log_ring_t *r)
{
    ...
    /* Iterator yields exactly count records, ascending gapless seqs. */
    log_ring_iter_t it;
    log_ring_view_t v;
...
/* ----- 3) Wrap-marker sweep --------------------------------------------- */
/* Small ring + all text lengths 0..47: the wrap thus lands on every possible
 * offset, incl. exact fit (no marker) and a 1-byte remaining gap. */
...
/* ----- 6) randomized stress against a shadow model ----------------------- */
typedef struct {
    uint32_t seq;
...
static void test_stress(void)
{
    ...
    static shadow_rec_t shadow[SHADOW_MAX];   /* ring buffer by seq index */
    srand(20260816);
