 ...
 * See datapoints.h for the architecture overview and DATAPOINTS.md for an
    ...
    switch (d->type) {
        ...
        case DP_U64:   /* accepted as decimal or hex string */
            ...
            { char *end = NULL;
              ...
              /* No digits consumed or garbage after them -> reject
               * instead of silently writing "0" ("hallo" -> error). */
        ...
        case DP_STR:
            if (!cJSON_IsString(v)) { *err = "type_mismatch"; return false; }
            /* REJECT overlong strings, do not silently truncate to DP_STR_MAX-1
             * (otherwise the device acknowledges "applied" for a truncated
             * value). */
...
bool dp_write_batch(const cJSON *dp_in, cJSON *errors_out, bool *pbPersisted)
{
    ...
    /* ENTIRE transaction under ONE lock hold: snapshot -> validation ->
     * cross-field constraints -> commit. If the mutex were released between
     * snapshot and commit (earlier state), two concurrent writers (cloud +
     * local, or two local sessions) could each validate against a stale
     * snapshot and JOINTLY violate the constraints, although each batch is
     * valid on its own (TOCTOU). Under the lock only CPU work runs
...
bool dp_constraints_ok(const dp_image_t *m, const char **bad)
{
    ...
    /* CANopen bitrate: only the classic CiA-301 rates the TWAI driver has
     * timing presets for (canopen_task.c). */
    switch (m->Can_Bitrate_img) {
        case 10: case 20: case 50: case 100: case 125: case 250: case 500: case 800: case 1000: break;
        default: *bad = "Can_Bitrate"; return false;
    }
    return true;
}
...
void dp_refresh(void)
{
    ...
    /* Largest contiguous block — fragmentation/leak metric for
     * stress observation (readable live via dp_read). */
