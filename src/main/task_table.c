...
static esp_err_t canopen_cycle_supervised(tm_task_ctx_t *pstCtx)
{
    esp_err_t err = canopen_task_cycle(pstCtx);
    if (err == ESP_OK) wd_heartbeat(WD_CH_CANOPEN);
    return err;
}
...
static const tm_periodic_task_def_t s_astDefs[TM_TASK_COUNT] = {
    ...
    [TM_TASK_CANOPEN] = {
        .name           = "canopen",
        .on_start       = NULL,          /* canopen_init runs in main_init  */
        .on_cycle       = canopen_cycle_supervised,
        .on_stop        = NULL,
        .user_arg       = NULL,
        .period_ms      = CANOPEN_TASK_PERIOD_MS,   /* 10 ms: SDO latency  */
        .max_runtime_ms = 500,           /* an SDO write may hit NVS        */
        .stack_size     = 6144,          /* cJSON dp_write path + TWAI      */
        .priority       = 5,
        .core_id        = 0,             /* core 1 stays with the control loop */
        .twdt_subscribe = true,
    },
};
