 ...
 *
 * factory_config — device-specific production data (series production).
 *
 ...
 * directly, only this interface — the storage thus remains replaceable
 * (later e.g. encrypted NVS, eFuse, DS peripheral).
 *
 * If the partition is missing or empty/incomplete, the compiled-in fallback
 * identity applies (bit-exact the previous testbed identity) — the same
 * firmware image thus runs unchanged on the existing device without a
 * factory partition (OTA compatibility, ota_0/ota_1 untouched).
 */
#pragma once
 ...
 * ESP_OK — missing/broken factory data is NOT a boot error, but leads
 * to the fallback identity (from_factory == false). */
esp_err_t factory_config_init(void);

/* Static instance living for the whole process — pointers into it may be
 * taken over directly into long-lived configurations (fp_config_t). */
