...
/* Voltage divider ratio V_adc/V_sensor: board-dependent (hal_pins).
 * Devkit: no divider (ADS1115 at 5 V, sensor max 4.5 V direct).
 * 24V PCB: 10k/15k before the ADS1115 (which runs at 3V3 there) -> 0.6. */
#define HAL_VDIV_RATIO     (hal_pins()->flSensorVdiv)
...
/* --- Runtime pin map (see hal.h) ---------------------------------------------
 * Keyed off the factory hw_revision. "HW2" prefix selects the
 * fountainer_hw_24v PCB; everything else (incl. missing factory partition)
 * keeps the DevKitC testbed wiring — bit-identical legacy behaviour. */
static const hal_pins_t s_stPinsHw1 = {
    .eDht = GPIO_NUM_4,  .eI2cSda = GPIO_NUM_8,  .eI2cScl = GPIO_NUM_7,
    .eSsr = GPIO_NUM_5,  .eLedRed = GPIO_NUM_NC, .eLedGreen = GPIO_NUM_NC,
    .flSensorVdiv = 1.0f,
    .eCanTx = GPIO_NUM_NC, .eCanRx = GPIO_NUM_NC, .eCanStby = GPIO_NUM_NC,
};
static const hal_pins_t s_stPinsHw2 = {
    /* IO6 is not routed on the 24V PCB — safe dummy for the (absent) DHT:
     * the driver toggles a dead pin, reads time out, task_measure treats it
     * as "sensor missing" exactly like an unplugged sensor on the devkit. */
    .eDht = GPIO_NUM_6,  .eI2cSda = GPIO_NUM_17, .eI2cScl = GPIO_NUM_18,
    .eSsr = GPIO_NUM_7,  .eLedRed = GPIO_NUM_15, .eLedGreen = GPIO_NUM_16,
    /* Input divider R17/R20 = 10k/15k -> 15/(10+15) (bring-up 2026-09-20,
     * FERTIGUNG_README addendum): without correction ~40 % too little pressure. */
    .flSensorVdiv = 15.0f / 25.0f,
    /* CAN (KiCad esp32.kicad_sch): CAN_TX = IO5, CAN_RX = IO4, CAN_PWDN =
     * IO21 -> TJA1051T/3 S pin (internal pull-up = standby when floating). */
    .eCanTx = GPIO_NUM_5, .eCanRx = GPIO_NUM_4, .eCanStby = GPIO_NUM_21,
};

const hal_pins_t *hal_pins(void)
{
    const char *pstrRev = factory_config_get()->hw_rev;
    return (strncmp(pstrRev, "HW2", 3) == 0) ? &s_stPinsHw2 : &s_stPinsHw1;
}

static bool hal_board_is_hw2(void)
{
    return hal_pins() == &s_stPinsHw2;
}

/* All GPIOs that carry no function on the board at hand, driven HIGH as
 * outputs. Defined levels (no floating inputs), and HIGH — not LOW — on
 * purpose: the devkit's status LEDs (TX/RX of the CH343 USB-serial bridge
 * on 43/44, and others) sit between 3V3 and the signal, i.e. ACTIVE-LOW.
 * Grounding the pins lit them all up (observed on this bench); HIGH keeps
 * them dark. 43/44 are UART0 and since the console moved there (CP2102N on
 * the 24V PCB has no other channel) they are NEVER parked any more — on the
 * devkit that merely lights the CH343 activity LEDs with console traffic.
 * NOT in the devkit list: 4 DHT, 5 SSR, 7/8 I2C, 19/20 USB-JTAG, 26..37
 ...
 * NOT in the HW2 list: 4/5 CAN RX/TX (transceiver push-pull drives RX —
 * never drive against it), 6 DHT dummy, 7 SSR, 15/16 LEDs, 17/18 I2C,
 * 21 CAN standby (driven high separately), 35..37 octal-PSRAM (N16R8!),
 * 39..42 JTAG header J4, 43/44 UART0 console via CP2102N. */
static void unused_pins_park_high(void)
{
    static const gpio_num_t aeUnusedHw1[] = {
        GPIO_NUM_1,  GPIO_NUM_2,  GPIO_NUM_6,  GPIO_NUM_9,  GPIO_NUM_10,
        GPIO_NUM_11, GPIO_NUM_12, GPIO_NUM_13, GPIO_NUM_14, GPIO_NUM_15,
        GPIO_NUM_16, GPIO_NUM_17, GPIO_NUM_18, GPIO_NUM_39, GPIO_NUM_40,
        GPIO_NUM_41, GPIO_NUM_42, GPIO_NUM_47,
    };
    static const gpio_num_t aeUnusedHw2[] = {
        GPIO_NUM_1,  GPIO_NUM_2,  GPIO_NUM_8,  GPIO_NUM_9,  GPIO_NUM_10,
        GPIO_NUM_11, GPIO_NUM_12, GPIO_NUM_13, GPIO_NUM_14, GPIO_NUM_19,
        GPIO_NUM_20, GPIO_NUM_38, GPIO_NUM_47, GPIO_NUM_48,
    };
    const gpio_num_t *aeUnused = hal_board_is_hw2() ? aeUnusedHw2 : aeUnusedHw1;
    size_t szCount = hal_board_is_hw2()
                         ? sizeof(aeUnusedHw2) / sizeof(aeUnusedHw2[0])
                         : sizeof(aeUnusedHw1) / sizeof(aeUnusedHw1[0]);
...
{
    gpio_num_t ePin = hal_pins()->eCanStby;
    if (ePin == GPIO_NUM_NC) return;
    gpio_reset_pin(ePin);
    gpio_set_direction(ePin, GPIO_MODE_OUTPUT);
    gpio_set_level(ePin, bStandby ? 1 : 0);
}

bool hal_can_available(void)
{
    return hal_pins()->eCanTx != GPIO_NUM_NC && hal_pins()->eCanRx != GPIO_NUM_NC;
}

bool hal_setup(void)
{
    logging(LOG_TARGET_AUTO, DBG_LVL_LOW, TAG, "init (board=%s)",
            hal_board_is_hw2() ? "HW2/fountainer_hw_24v" : "HW1/devkit");

    if (!hal_board_is_hw2()) {
        /* Devkit: WS2812 sits on 48, 38 or 21 depending on board revision —
         * blank all three (0,0,0 = off), they stay driven LOW (data idle). */
        rgb_led_blank(GPIO_NUM_48);
        rgb_led_blank(GPIO_NUM_38);
        rgb_led_blank(GPIO_NUM_21);
    } else {
        /* 24V PCB: IO21 is the TJA1051T/3 S pin (net CAN_PWDN). Hold the
         * transceiver in silent mode until the CANopen task has the TWAI
         * driver running (canopen_task.c -> hal_can_standby_set(false)). */
        hal_can_standby_set(true);
    }
    unused_pins_park_high();

    /* Status LEDs (HW2.x only; no-op without LED pins): both on for 5 s at
     * power-up, then the green one signals server communication (1 Hz). */
    status_led_init();
...
bool hal_relay_set(bool bOn)
{
    ...
    s_relay_on = bOn;
    /* Status LED: green blinks as long as the SSR is switched on (idempotent —
     * the pump task calls relay_set every cycle). */
    status_led_relay_active(bOn);
