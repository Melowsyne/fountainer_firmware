...
| Domain logic | `src/device/` | `pump_manager` (pure FSM, host-tested), `pump_task`, `task_measure`, `command`, `canopen_task` (TWAI binding of the CANopen core), `hal`, `onewire_am2302` |
| Connectivity | `src/network/` | `wlan_com`, `task_com`, `ota_task`, `network_config`, `link_quality`, `link_score`, `net_utils` |
| Framework components | `src/components/` | `clientside_protocol`, `datapoints`, `event_manager`, `task_manager`, `logging`, `watchdog`, `canopen` (pure CiA-301 slave core) |
| Tooling | `tools/` | data-point lint + UI meta export, CANopen EDS generator, cert/network embedding, build timestamp, OTA signing |
...
| `canopen` | 5 | 0 | 10 ms | CANopen slave: TWAI rx, SDO/PDO/heartbeat ([DOKU/CANopen.md](DOKU/CANopen.md)) |
...
| `Fon_Pump_Command` | — | U8 RW (0–5) | Command point, always reads 0: 1 = On, 2 = Off, 3 = mode Auto, 4 = mode Manual, 5 = restart (same rules as `set_state`; also the CAN control point). |
...
#### CANopen slave (config RW NVS ids 601–605; status VOLATILE — [DOKU/CANopen.md](DOKU/CANopen.md))

| Datapoint | NVS id | Type | Purpose |
|---|---|---|---|
| `Can_Enabled` | 601 | BOOL (default 1) | Master switch for the CANopen slave (HW2.x transceiver). |
| `Can_Node_Id` | 602 | U8 (default 3, 1–127) | CANopen node id; all COB-IDs derive from it. |
| `Can_Bitrate` | 603 | U16 (default 250) | Bus bitrate in kbit/s: 10/20/50/100/125/250/500/800/1000. |
| `Can_Heartbeat_Ms` | 604 | U16 (default 1000) | Heartbeat producer period, 0 = off (object 0x1017). |
| `Can_Loopback` | — | BOOL RW | Bench self-test: TWAI no-ACK mode (own frames need no partner). |
| `Can_Master_Timeout_Ms` | 605 | U16 (default 5000) | Master presence window for `Can_Master_Active`. |
| `Can_Master_Active` | — | BOOL RO | 1 while a master addressed this node within the timeout (edge-logged). |
| `Can_Master_Age_S` | — | U32 RO | Seconds since the last master frame (0xFFFFFFFF = never). |
| `Can_Nmt_State` | — | U8 RO | Raw NMT state: 0, 4 stopped, 5 operational, 127 pre-operational. |
| `Can_Sdo_Count` / `Can_Pdo_Tx_Count` / `Can_Pdo_Rx_Count` | — | U32 RO | SDO requests served, TPDOs sent, RPDOs applied since boot. |
| `Can_Emcy_Count` | — | U16 RO | Emergencies sent since boot. |
| `Can_State` | — | ENUM RO | 0 off/no hardware, 1 initialising, 2 pre-operational, 3 operational, 4 stopped, 5 bus-off/paused. |
| `Can_Rx_Frames` / `Can_Tx_Frames` | — | U32 RO | Frames received / sent since boot. |
| `Can_Bus_Errors` | — | U32 RO | Bus errors + failed transmissions since boot. |
| `Can_Bus_Off_Count` | — | U16 RO | Bus-off events since boot. |
...
of all 126 datapoints, grouped as in `dp_list.def` (collapsible).*
...
The certificate authority lives **outside the repository** (`../CA`):
...
client certificate would break that server role (incident 16.08.2026).
...
test/host/run.sh                              # pump_manager FSM, link scorer,
                                              # log_ring, pressure_ring, co_core
                                              # (CANopen), datapoints
...
| [DOKU/CANopen.md](DOKU/CANopen.md) | CANopen slave: object dictionary (datapoints 0x2000+), PDOs, EMCY, master quick start, EDS in `DOKU/canopen/` |
