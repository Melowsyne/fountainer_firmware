 ...
 * task_com.c:388ff and main.c:363) — behaviour without a factory partition
 * stays identical to the existing one. */
static void defaults_set(void)
{
    ...
    strlcpy(s_stFactory.server_host, "SERVER_IP", sizeof s_stFactory.server_host);
     ...
     * factory partition. TODO production hardening: flash-encrypted
     * NVS or HMAC eFuse block. */
...
esp_err_t factory_config_init(void)
{
    ...
    if (e != ESP_OK) {
        /* Partition missing (old table) or corrupt: NEVER erase —
         * the production data is not reproducible. The fallback applies. */
    ...
    /* Optional individual values — if missing, the respective default is kept. */
    ...
    /* Certificate + key only as a PAIR — with only one present both stay empty
     * and the embedded fallbacks apply (otherwise mTLS with half the material).
     * Read directly into the static instance (3 KB buffers do not belong on
     * the init task stack); reset on a half pair. */
