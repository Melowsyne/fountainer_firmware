 ...
 * instead of fixed 84 B slots — the same 32 KiB budget thus holds roughly
 * 700–1100 records instead of 390. boot_id is ring metadata, not part of the
 * record (Entwurf §27); externally log_record_t remains the copy-out type,
 * so that flash format and protocol stay byte-identical. */
...
{
    /* STORE GATE (reactivated 23.08.): Log_Runtime_Level again limits
     * what is stored in the RAM ring (default INFO). Background: the
     * 16.08. full capture (all levels) produced ~16 records/s — the ring
     * (~800 records) permanently overflowed between the 60 s server polls,
     * 14-95 %% of the records were lost as gaps (Log_Dropped in the
     * millions). DEBUG/TRACE remain RETRIEVABLE: at runtime set
     * Log_Runtime_Level to 4/5 via dp_write, then everything is stored
     * again. */
                  ...
                  const int32_t *pslArgs, uint8_t ucArgc, const char *pstrText)
{
    ...
    /* Serialisation + bounded eviction under the short lock; the seq
     * is assigned by the ring. */
...
{
    /* Again controls the STORE GATE of the RAM ring (records with a level
     * above are not stored) — see logging_is_enabled_fast. */
