...
Strictly additive: existing entries (in particular the production device
esp32-a1b2c3d4e5f6) are NEVER modified; an already registered
device_id aborts. The registry is only read at server start
...
def main():
    ...
    print("Restart required: cd ../fountainer_server && bash stop.sh && bash start.sh")
    print("(The production device goes offline for ~15 s and reconnects —")
    print(" check in the admin web UI that it is back online before continuing.)")
