 ...
 * is the hard RAM bollard (implementation plan: start with 2, 5 only after
...
/* Concurrent local maintenance sessions. Default 1: in practice a
 * maintenance interface serves one client; this keeps RAM free (~18 KB per
 * session with MBEDTLS_DYNAMIC_BUFFER) and categorically rules out two
 * concurrent LOCAL writers. Can be raised at build time via
 * -DLOCAL_SERVER_MAX_CLIENTS=<n> (1..5) (M1: RAM is the bottleneck; measure
 * >2 before release). CAUTION with 1: if a client hangs half-open, the
 * device is locally unreachable until the idle timeout (LOCAL_IDLE_TIMEOUT_US, 5 min). */
...
typedef struct {
    ...
    uint32_t rate_drops;
    uint32_t auth_failures;       /* TLS level counted by esp-tls; Fountain here */
