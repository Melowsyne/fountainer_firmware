...
/* Status-LED (HW2.x): green is lit solid while the server session is up
 * (blinking while the SSR is switched is driven from hal_relay_set).
 * "Up" = hello_ack accepted; any session loss or WLAN drop stops it. */
static void glue_status_led(system_event_t eEvent, const void *pvData,
                            size_t szSize)
{
    (void)pvData; (void)szSize;
    status_led_comm_active(eEvent == EVT_SESSION_READY);
}
...
esp_err_t main_monitor_cycle(tm_task_ctx_t *pstCtx)
{
     ...
     * task_manager — data-structure/stack safety for long-term operation.
     * Covers the managed tasks (incl. the 200-ms pump task); local_server
     * worker/httpd are raw xTaskCreate and not captured here. */
    {
    ...
    /* Pressure history: mirror ring stats for server diagnostics. */
    {
...
static bool main_init(void)
{
    ...
    /* CANopen slave (HW2.x transceiver; no-op on the devkit): datapoints as
     * object dictionary 0x2000+, TPDOs, SDO writes via the dp_write path. */
    INIT_STEP(canopen_init());
    ...
    /* Status-LED follows the session state (no-op on boards without LEDs). */
    event_manager_subscribe(EVT_SESSION_READY, glue_status_led);
    event_manager_subscribe(EVT_SESSION_LOST, glue_status_led);
    event_manager_subscribe(EVT_WLAN_DISCONNECTED, glue_status_led);
