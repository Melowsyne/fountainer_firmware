...
--port is mandatory (no auto-detection — mix-up protection: only the
production/test device on the given port may be flashed, never a device
in the field). Status transition: provisioned/flashed/failed -> flashed.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    ap.add_argument("--erase-all", action="store_true",
                    help="Erase the whole chip first (factory-new devices)")
    args = ap.parse_args()
    ...
    # Check artifacts against the manifests — only verified material is flashed.
    ...
    # default_reset: esptool handles the bootloader entry via USB-Serial-JTAG
    # itself (verified on the testbed); if the sequence hangs, manually
    # holding BOOT + RESET helps (then no_reset does not matter).
