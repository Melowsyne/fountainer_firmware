...
/* Upper limit for TIME-LIMITED manual switch-on (turn_on_duration).
 * An active timed-on is decoupled from the Fon_Max_On_Time fault limit: it
 * switches off cleanly at its deadline (no MAX_RUNTIME fault) but is
 * hard-clamped to this ceiling. Fon_Max_On_Time remains the safety limit
 * for AUTO and unlimited manual on. */
