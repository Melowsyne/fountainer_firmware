...
/* Run time since pump_on() WITHOUT underflow: remote requests (pm_request_on
 * from the command context) stamp pump_start_ms with a FRESHER now_ms() than
 * the ullNow the running pump_task cycle fetched at cycle start. The naive
 * difference then wrapped unsigned (~2^64 ms) and the max-runtime protection
 * latched immediately on switch-on (stress test finding 2026-09-23, web/C++
 * path; the CAN path runs inside the cycle and was never affected). */
static uint64_t run_ms(const pm_t *pm, uint64_t ullNow)
{
    return ullNow > pm->pump_start_ms ? ullNow - pm->pump_start_ms : 0;
}
...
void pm_update(pm_t *pm, float flBar, bool bSensorOk, uint64_t ullNow)
{
    ...
    if (pm->state != PM_STATE_FAULT) {
        if (!bSensorOk) {
            ...
            uint64_t ullRun = run_ms(pm, ullNow);
    ...
    if (pm->relay_on) {
        uint32_t ulRunS = (uint32_t)(run_ms(pm, ullNow) / 1000ULL);
        ...
        if (pm->mode == PM_MODE_MANUAL && pm->manual_until_ms > pm->pump_start_ms) {
             ...
             * duration (decoupled from max_on_ms; the duration is already
