 ...
 *   B) Atomic all-or-nothing batch: one error -> nothing committed.
 *   C) Concurrency stress (F1): two threads write overlapping values to
 *      cross-field-constrained DPs, a checker thread continuously reads
 ...
 * is COPIED into a static buffer (cJSON is freed afterwards). */
...
static void test_validation(void)
{
     ...
     * what counts here: rejected + store unchanged (Min stays 2.0). */
...
/* Continuously writes ONE-SIDED Min or Max with overlapping values —
